...
from searchtweets import searchtweetsbykeyword, searchtweetsbyTweet
...
#training_set1 = nltk.classify.util.apply_features(extract_features, tweets, labeled=True)
...
SklearnClassifier = nltk.classify.scikitlearn.SklearnClassifier(SVC()).train(training_set)

print 'Done Classification'

def analyze(keyword):
    tweetstream = searchtweetsbyTweet(keyword)
   # tweetstream = searchtweetsbykeyword(keyword)
    ...
    for testTweet in tweetstream:
        ...
        entry.append(sentiment)
        sentiment = SklearnClassifier.classify(extract_features(getFeatureVector(processedTestTweet)))
        entry.append(sentiment)
